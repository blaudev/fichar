...
async function getCalendarApi(): Promise<calendar_v3.Calendar> {
 ...
 */
export async function createDayEvents(schedule: DaySchedule): Promise<void> {
  const cal = await getCalendarApi();
  const calendarId = await getOrCreateCalendar();

  const entries: { title: string; start: Date }[] = [
...
export async function getTodaySchedule(): Promise<DaySchedule | null> {
  ...
  const find = (title: string): Date | null => {
  ...
  if (!clockIn || !pause || !resume || !clockOut) return null;
 ...
 */
export async function clearDayEvents(date: Date): Promise<number> {
  const cal = await getCalendarApi();
  const calendarId = await getOrCreateCalendar();
