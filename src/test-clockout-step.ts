  ...
  const btns = await page.evaluate(() => {
    return [...document.querySelectorAll("a[id], button[id], input[type='submit'][id], input[type='button'][id]")]
      .map((el) => {
        ...
        const visible = s.display !== "none" && s.visibility !== "hidden" && (el as HTMLElement).offsetWidth > 0;
        return {
          id: el.id,
          tag: el.tagName,
...
async function infoSelector(page: Page, selector: string): Promise<void> {
  const info = await page.evaluate((sel) => {
    ...
    return {
      tag: el.tagName,
      id: el.id,
      text: (el.textContent || (el as HTMLInputElement).value || "").trim().substring(0, 80),
      visible: s.display !== "none" && s.visibility !== "hidden" && (el as HTMLElement).offsetWidth > 0,
      disabled: (el as HTMLInputElement).disabled ?? false,
...
async function clickAndReport(page: Page, selector: string, label: string): Promise<void> {
...
const timeout = new Promise<never>((_, reject) =>
  setTimeout(() => reject(new Error(`⏰ Timeout global (${TIMEOUT_MS / 1000}s) alcanzado`)), TIMEOUT_MS),
);
