...
import { clearDayEvents, createDayEvents, type DaySchedule } from "./google-calendar";
...
function loadHolidays(): Set<string> {
  const data: Holiday[] = JSON.parse(readFileSync(HOLIDAYS_PATH, "utf-8"));
 ...
 */
function generateDaySchedule(date: Date, weekExcessMs = 0): DaySchedule {
...
function getTotalWorkMs(s: DaySchedule): number {
...
async function main() {
  ...
  while (current <= endDate) {
    ...
    if (isWeekday(current)) {
      ...
      console.log(
        `   ${formatDate(current)}: ${formatTime(schedule.clockIn)} → ${formatTime(schedule.pause)} | ${formatTime(schedule.resume)} → ${formatTime(schedule.clockOut)}  (${(workMs / 3600000).toFixed(2)}h)`,
      );

      if (clearFirst) {
        const removed = await clearDayEvents(current);
        if (removed > 0) console.log(`      🗑️ ${removed} eventos eliminados`);
      }
