...
async function dumpButtons(page: Page) {
  const elements = await page.evaluate(() => {
    ...
    return els.map((e) => {
      ...
      return {
        ...
        href: (e as HTMLAnchorElement).href ? (e as HTMLAnchorElement).href.substring(0, 120) : null,
  ...
  elements.forEach((e, i) =>
    console.log(
      `   ${i + 1}. [${e.visible ? "VISIBLE" : "OCULTO"}] <${e.tag}> id="${e.id}" text="${e.text}"\n      HTML: ${e.outerHTML}`,
    ),
  );
  return elements;
}

async function tryClick(page: Page, selector: string, label: string): Promise<boolean> {
  ...
  try {
    ...
    const posted = await page.evaluate((sel) => {
      ...
      if (href.includes("__doPostBack")) {
        ...
        if (match) {
          (window as any).__doPostBack(match[1], "");
...
async function main() {
  ...
  try {
      ...
      const altConfirm = await page.evaluate(() => {
        ...
        return btns
          .filter((b) => {
            ...
            return (
              (text.includes("finalizar") || text.includes("confirmar") || text.includes("aceptar")) &&
    ...
    let confirm2: import("playwright").ElementHandle | null = null;
    try {
