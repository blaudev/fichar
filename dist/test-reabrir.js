...
async function main() {
    ...
    try {
        ...
        await page.evaluate(() => window.DoFichaje("REABRIR"));
        await sleep(env.postActionWait);
        // Buscar qué hay en la página tras la llamada
        const html = await page.content();
        const visibleButtons = await page.$$eval("button:visible, input[type='button']:visible, input[type='submit']:visible, a.btn:visible", (els) => els.map((el) => ({ tag: el.tagName, id: el.id, text: el.textContent?.trim(), className: el.className })));
        ...
        const modals = await page.$$eval("[class*='modal']:visible, [class*='dialog']:visible, [class*='popup']:visible, [class*='alert']:visible", (els) => els.map((el) => ({ id: el.id, className: el.className, text: el.textContent?.trim().substring(0, 200) })));
