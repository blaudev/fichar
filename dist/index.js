...
function loadWeekLog() {
    ...
    if (existsSync(WEEK_LOG_PATH)) {
        const data = JSON.parse(readFileSync(WEEK_LOG_PATH, "utf-8"));
...
function loadDayState() {
    if (!existsSync(DAY_STATE_PATH))
        return null;
    try {
        const data = JSON.parse(readFileSync(DAY_STATE_PATH, "utf-8"));
...
async function screenshotOnError(page, phase) {
    try {
...
async function verifyNextState(page, expectedSelector, phase) {
    await page.waitForLoadState("networkidle");
    await sleep(env.postActionWait);
...
async function performAction(page, label, selector, confirmSelector, secondConfirmOptional = false) {
